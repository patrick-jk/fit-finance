...
public interface FinanceRepository extends JpaRepository<Finance, Long> {
    ...
    @Query("SELECT f.user.income + SUM(CASE WHEN f.type = 'INCOME' THEN f.value ELSE 0 END) - SUM(CASE WHEN f.type = 'EXPENSE' THEN f.value ELSE 0 END) " +
    ...
    BigDecimal getExpensesTotalByUserId(Long userId);
}